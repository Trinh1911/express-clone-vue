        ...
        <div class="md:w-[35%]">
          <div id="PlaceOrder" class="bg-white rounded-lg p-4">
            ...
            <form @submit.prevent="pay()">
              ...
              <button
                type="submit"
                class="mt-4 bg-gradient-to-r from-[#FE630C] to-[#FF3200] w-full text-white text-[21px] font-semibold p-1.5 rounded-full opacity-100"
              >
...
let timePart = ref(null);
console.log('user.value', user.value.created_at)
onBeforeMount(async () => {
...
const pay = async () => {
  const timePart = user.value.created_at.substring(
    user.value.created_at.lastIndexOf(".") + 1
  );
  const idOrder =  timePart + user.value.id
  await createOrder(idOrder);
  userStore.cart = [];
  userStore.checkout = [];
